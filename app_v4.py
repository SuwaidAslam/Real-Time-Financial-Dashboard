...
y_axis_names = ['', 'Long Market Value Stock', 'Day-trading Buying Power', 'Maintenance Margin', 'Short Market Value Stock',
                'Regulation T Buying Power', 'Initial Margin']

y_axis_col_names = ['pl_last_equity', 'long_market_value_stock',
                'daytrading_buying_power_last_equity', 'maintenance_margin_last_equity',
                'short_market_value_stock', 'regt_buying_power_last_equity',
                'initial_margin_last_equity']
...
def fig(n_intervals, date_value):
    ...
    all_figures = make_subplots(
        ...
        specs=[[{"colspan": 3}, None, None],
           ...
           [{}, {}, {}]], 
            ...
            subplot_titles=y_axis_names
        )
        
    subPlots_counter = 1
    for y_axis_col_name in y_axis_col_names:
        if subPlots_counter == 7:
            legandStatus = True
        else:
            legandStatus = False
    ...
    all_figures.update_layout(height=900,  uirevision= 'data', hovermode="closest",
    font= dict(
    size=10,
    color="#000000"
    ),
...
def plotTreeMap(n_intervals, date_value, dropdown_value, hour_value, minute_value, mode):
    ...
    try:
        ...
        if stocks_id == None:
            ...
            query =  '''Select entry_id From {0} Where DATE(ts) = '{1}' AND HOUR(ts) = '{2}' AND MINUTE(ts) = '{3}'
                    ORDER BY entry_id DESC LIMIT 1;
                    '''.format(current_position_table , str(date_value), str(hour_value), str(m))
            cursor.execute(query)
            stocks_id = cursor.fetchone()
